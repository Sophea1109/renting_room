...
const { fetchOwnerDashboard, updateOwnerRoom, deleteOwnerRoom } = ownerDashboardApi;
...
export default function RoomsPage() {
  ...
  const filteredList =
    categoryFilter === "hotel"
      ? roomsList.filter((r) => r.type === "hotel")
      : categoryFilter === "room"
      ? roomsList.filter((r) => r.type !== "hotel")
      : roomsList;
  ...
  const tabCounts = {
    ...
    room: roomsList.filter((r) => r.type !== "hotel").length,
    hotel: roomsList.filter((r) => r.type === "hotel").length,
  };
  ...
  const handleInputChange = (e) => {
    ...
    if (name.startsWith("owner.")) {
      ...
      setFormData((prev) => ({ ...prev, owner: { ...prev.owner, [ownerField]: value } }));
    } else {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
        ...
        prev.map((room) =>
          room.id !== roomToEdit.id
            ...
            : {
                ...
                owner: { ...room.owner, name: formData.owner.name, contact: formData.owner.contact },
              }
        )
      );
      ...
      setActionError(error?.message || "Failed to update room");
    } finally {
      setIsEditing(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setRoomsList((prev) => prev.filter((room) => room.id !== roomToDelete.id));
  ...
  const cancelDelete = () => { setShowDeleteModal(false); setRoomToDelete(null); };
  const cancelEdit = () => { setShowEditModal(false); setRoomToEdit(null); };
    ...
    <div className="flex h-screen overflow-hidden bg-slate-900 text-slate-100">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
        ...
        <main className="p-6 flex-1 overflow-y-auto bg-gray-50 relative">
          ...
          {actionError && (
            <div className="absolute top-6 left-1/2 -translate-x-1/2 z-50 animate-slide-in">
                ...
                {filteredList.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-gray-400 text-sm">
                      No {categoryFilter === "hotel" ? "hotels" : categoryFilter === "room" ? "rooms" : "listings"} found.
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={room.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          <div className="flex-shrink-0 h-12 w-16 mr-4 overflow-hidden rounded-md bg-gray-200 flex items-center justify-center">
                            {room.image
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-medium text-gray-900">{room.title}</span>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          <div className="flex-shrink-0 h-8 w-8 mr-3 rounded-full bg-gray-200 flex items-center justify-center">
                          ...
                          <div>
                            ...
                            {room.owner.contact && (
                              <div className="text-xs text-gray-500 flex items-center">
                                <FiMail className="w-3 h-3 mr-1" />{room.owner.contact}
                              </div>
                            )}
                          </div>
                        </div>
